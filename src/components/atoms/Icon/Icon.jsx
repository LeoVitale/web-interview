import React from 'react'
import PropTypes from 'prop-types'
import { svgIcon } from './Icon.module.scss'

const Icon = ({ type, className }) => {
  let vector
  switch (type) {
    case 'user':
      vector =
        'M4.78 20.547h14.317c-.63-3.922-2.93-5.797-7.078-5.797s-6.505 1.879-7.24 5.797zm-1.618.66c.637-5.24 3.659-7.957 8.857-7.957 5.205 0 8.17 2.728 8.676 7.975a.75.75 0 0 1-.747.822H3.906a.75.75 0 0 1-.744-.84zM12 11.75a4.75 4.75 0 1 1 0-9.5 4.75 4.75 0 0 1 0 9.5zm0-1.5a3.25 3.25 0 1 0 0-6.5 3.25 3.25 0 0 0 0 6.5z'
      break
    case 'arrow':
      vector =
        'M16.216 17.223a.75.75 0 1 0 1.068 1.054l5.928-6.01a.75.75 0 0 0-.001-1.054l-5.928-5.99a.75.75 0 1 0-1.066 1.055l5.407 5.463-5.408 5.482z'
      break
    case 'appointments':
      vector =
        'M3.777 4.75a.25.25 0 0 0-.25.25v14c0 .138.112.25.25.25h16.55a.25.25 0 0 0 .25-.25V5a.25.25 0 0 0-.25-.25H3.777zm0-1.5h16.55c.966 0 1.75.784 1.75 1.75v14a1.75 1.75 0 0 1-1.75 1.75H3.777A1.75 1.75 0 0 1 2.027 19V5c0-.966.784-1.75 1.75-1.75zm3.164-.449a.75.75 0 1 1 1.5 0v2.987a.75.75 0 0 1-1.5 0V2.8zm8.566 0a.75.75 0 1 1 1.5 0v3.04a.75.75 0 1 1-1.5 0v-3.04zM2.75 9.75v-1.5h18.5v1.5H2.75zm12.5 4v2.5h2.5v-2.5h-2.5zm-.75-1.5h4a.75.75 0 0 1 .75.75v4a.75.75 0 0 1-.75.75h-4a.75.75 0 0 1-.75-.75v-4a.75.75 0 0 1 .75-.75z'
      break
    case 'family':
      vector =
        'M17 12.75a2.75 2.75 0 1 1 0-5.5 2.75 2.75 0 0 1 0 5.5zm0-1.5a1.25 1.25 0 1 0 0-2.5 1.25 1.25 0 0 0 0 2.5zm1.1 3.898a.75.75 0 0 1 .8-1.268c1.307.825 2.868 3.612 3.105 5.465a.75.75 0 0 1-.743.845h-2.334a.75.75 0 1 1 0-1.5h1.39c-.448-1.385-1.48-3.075-2.219-3.542zm-3.007 3.547c-.55-2.889-2.378-4.26-5.665-4.26s-5.162 1.374-5.793 4.26h11.458zm-13.09.655c.535-4.218 3.084-6.416 7.425-6.416 4.348 0 6.85 2.208 7.276 6.436a.75.75 0 0 1-.747.825H2.747a.75.75 0 0 1-.745-.845zm7.409-7.284c-2.227 0-4.044-1.743-4.044-3.908 0-2.166 1.817-3.908 4.044-3.908 2.226 0 4.044 1.742 4.044 3.908 0 2.165-1.818 3.908-4.044 3.908zm0-1.5c1.412 0 2.544-1.086 2.544-2.408 0-1.323-1.132-2.408-2.544-2.408C8 5.75 6.868 6.835 6.868 8.158c0 1.322 1.132 2.408 2.544 2.408z'
      break
    case 'notes':
      vector =
        'M4.64 14.606a.75.75 0 0 1 .341-.083H17a.25.25 0 0 0 .25-.25V5a.25.25 0 0 0-.25-.25H3.769a.25.25 0 0 0-.25.25v10.18l1.12-.574zm-1.53 2.468a.75.75 0 0 1-1.091-.667V5c0-.966.783-1.75 1.75-1.75H17c.966 0 1.75.784 1.75 1.75v9.273a1.75 1.75 0 0 1-1.75 1.75H5.162L3.11 17.074zm16.292.676l1.121.574V7.654a.75.75 0 1 1 1.5 0V19.55a.75.75 0 0 1-1.092.667l-2.051-1.05H6.83a.75.75 0 1 1 0-1.5h12.23a.75.75 0 0 1 .342.082z'
      break
    case 'next':
      vector =
        'M13.25 10l-7.141-7.42c-0.268-0.27-0.268-0.707 0-0.979 0.268-0.27 0.701-0.27 0.969 0l7.83 7.908c0.268 0.271 0.268 0.709 0 0.979l-7.83 7.908c-0.268 0.271-0.701 0.27-0.969 0s-0.268-0.707 0-0.979l7.141-7.417z'
      break
    case 'add':
      vector =
        'M12.75 12.75v6.513a.75.75 0 1 1-1.5 0V12.75H4.74a.75.75 0 1 1 0-1.5h6.51V4.713a.75.75 0 1 1 1.5 0v6.537h6.568a.75.75 0 1 1 0 1.5H12.75z'
      break
    case 'clock':
      vector =
        'M12 21.75c-5.385 0-9.75-4.365-9.75-9.75S6.615 2.25 12 2.25s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75zm0-1.5a8.25 8.25 0 1 0 0-16.5 8.25 8.25 0 0 0 0 16.5zm4.128-5.118a.75.75 0 1 1-1.07 1.051l-3.808-3.876V7.044a.75.75 0 1 1 1.5 0v4.65l3.378 3.438z'
      break
    case 'close':
      vector =
        'M13.06 12.029l6.085 6.06a.75.75 0 0 1-1.058 1.062l-6.09-6.064-5.986 5.961a.75.75 0 1 1-1.059-1.063l5.983-5.956-6.03-6.005A.75.75 0 0 1 5.963 4.96l6.035 6.01 6.112-6.087a.75.75 0 0 1 1.059 1.063L13.06 12.03z'
      break
    case 'book':
    default:
      vector =
        'M19.504 17.997h1.75a.75.75 0 0 1 0 1.5h-1.75v1.75a.75.75 0 1 1-1.5 0v-1.75h-1.75a.75.75 0 1 1 0-1.5h1.75v-1.75a.75.75 0 0 1 1.5 0v1.75zM3.527 8.25h17.05V5a.25.25 0 0 0-.25-.25h-3.32v1.092a.75.75 0 1 1-1.5 0V4.75H8.441v1.038a.75.75 0 0 1-1.5 0V4.75H3.777a.25.25 0 0 0-.25.25v3.25zm17.05 1.5H3.527V19c0 .138.112.25.25.25h9.526a.75.75 0 1 1 0 1.5H3.777A1.75 1.75 0 0 1 2.027 19V5c0-.966.784-1.75 1.75-1.75h3.164v-.449a.75.75 0 1 1 1.5 0v.449h7.066v-.449a.75.75 0 1 1 1.5 0v.449h3.32c.966 0 1.75.784 1.75 1.75v9.601a.75.75 0 1 1-1.5 0V9.75z'
      break
  }

  return (
    <svg
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="#400099"
      role="img"
      className={`${svgIcon} ${className}`}
    >
      <title />
      <path d={vector} />
    </svg>
  )
}

Icon.propTypes = {
  type: PropTypes.string.isRequired,
  className: PropTypes.string.isRequired,
}

Icon.defaultProps = {
  type: '',
  className: '',
}

export default Icon
